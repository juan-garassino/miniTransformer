...
def get_stats(vocab: Dict[str, int],
              verbose: bool = False) -> Dict[Tuple[str, str], int]:
    pairs = Counter()
...
def get_tokens_from_vocab(
        vocab: Dict[str, int],
        verbose: bool = False) -> Tuple[Dict[str, int], Dict[str, List[str]]]:
...
def tokenize_bpe_word(word: str,
                      vocab_tokenization: Dict[str, List[str]]) -> List[str]:
    ...
    while i < len(word):
        ...
        for token_length in range(max_token_length, 0, -1):
            ...
            if potential_subword in vocab_tokenization:
                ...
                break
        if subword is None:  # if no subword has been found, treat the character as a token
...
def tokenize_bpe_sentence(sentence: str,
                          vocab_tokenization: Dict[str, List[str]],
    ...
    while i < len(sentence):
        ...
        for j in range(i + 1, len(sentence) + 1):
            ...
            if subword in vocab_tokenization and len(subword) > len(
                    max_subword):
                max_subword = subword
        if max_subword:
...
def tokenize_bpe(
        text: List[str],
        num_merges: int,
        verbose: bool = False) -> Tuple[Dict[str, int], List[List[str]]]:
    ...
    tokenized_text = [
        tokenize_bpe_sentence(s, vocab_tokenization) for s in text
    ]
    return tokens_frequencies, tokenized_text


def save_tokenizer(tokenizer: Tuple[Dict[str, int], List[List[str]]],
                   save_path: str) -> None:
...
def main():
    ...
    tokenized_sentence = tokenize_bpe_sentence(example_sentence,
                                               vocab_tokenization,
                                               verbose=True)
    ...
    # Tokenize the file
    tokenized_file = [
        tokenize_bpe_sentence(sentence, vocab_tokenization)
    ...
    re_tokenized_sentence = tokenize_bpe_sentence(example_sentence,
                                                  loaded_vocab_tokenization,
                                                  verbose=True)
...
def test():
    ...
    sentence_tokens = tokenize_bpe_sentence(example_sentence,
                                            vocab_tokenization)
    print("\nTokenized sentence: ", sentence_tokens)

    # Step 7: Full BPE tokenization (10 merges)
    tokens_frequencies, tokenized_text = tokenize_bpe([example_sentence],
                                                      num_merges=10)
